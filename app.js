(() => {
  ...
  const BASE_DATA_URLS = ['./data/today.json', 'data/today.json'];
  const DAILY_OVERLAY_URLS = ['./data/daily-2026-07-11.json', 'data/daily-2026-07-11.json'];
  ...
  function mergeData(overlay, base) {
    ...
    const groupKeys = new Set([...Object.keys(base.groups || {}), ...Object.keys(overlay.groups || {})]);
    const groups = {};
    groupKeys.forEach(key => { groups[key] = [...(overlay.groups?.[key] || []), ...(base.groups?.[key] || [])]; });
    return {
      ...base,
      ...overlay,
  ...
  async function loadData() {
    try {
      const [baseResult, overlayResult] = await Promise.all([
        fetchFirst(BASE_DATA_URLS),
        fetchFirst(DAILY_OVERLAY_URLS, true)
      ]);
      ...
      if (cached && cached.data) { STATE.source = 'local-cache'; return validateData(cached.data); }
      throw error;
    }
  }

  function setEngineStatus(mode, text) { const el = $('engineStatus'); if (!el) return; el.className = `engine-status ${mode}`; el.textContent = text; }
  ...
  function renderTabs() {
    ...
    CATEGORY_ORDER.forEach(cat => {
      ...
      button.textContent = CATEGORY_LABELS[cat] || cat; button.type = 'button';
  ...
  function renderMessages() {
    ...
    const items = selectedMessages(); grid.innerHTML = '';
  ...
  async function writeClipboard(text) {
    ...
    document.body.appendChild(area); area.select(); const ok = document.execCommand('copy'); document.body.removeChild(area);
  ...
  async function copyMessage(m, key, btn) {
    try {
      ...
      setStore('jeffreyUsage', used); btn.textContent = '✅ 已 Copy'; setTimeout(renderMessages, 700);
  ...
  function toggleFav(m, key) {
    ...
    setStore('jeffreyFavourites', fav); renderMessages();
  }

  function updateStats(visible) {
    const all = STATE.data ? flattenMessages(STATE.data) : [];
    $('totalCount').textContent = all.length; $('visibleCount').textContent = visible;
  ...
  function showRandomReminder() {
    const items = selectedMessages(); if (!items.length) return;
    const m = items[Math.floor(Math.random() * items.length)]; STATE.query = m.topic || m.content || '';
    $('searchInput').value = STATE.query; renderMessages(); window.scrollTo({ top:$('messageGrid').offsetTop - 20, behavior:'smooth' });
  }
  ...
  function toggleTheme() { const next = document.documentElement.dataset.theme === 'dark' ? 'light' : 'dark'; document.documentElement.dataset.theme = next; setStore('jeffreyTheme', next); $('themeToggle').textContent = next === 'dark' ? 'Light Mode' : 'Dark Mode'; }

  async function boot() {
    initTheme();
    $('searchInput').addEventListener('input', e => { STATE.query = e.target.value; renderMessages(); });
    $('randomReminder').onclick = showRandomReminder; $('themeToggle').onclick = toggleTheme;
    try {
      STATE.data = await loadData(); renderBrief(); renderTabs(); renderMessages();
      ...
      console.error('Jeffrey Reminder boot failure:', error); setEngineStatus('error-state', 'Engine error');
      ...
      $('messageGrid').innerHTML = '<div class="empty-state">網站框架正常，但未能讀取提醒資料。</div>'; updateStats(0);
    }
  }
