...
test('data model contains every approved production field', () => {
  ...
  assert.deepEqual(Object.keys(item), [
    ...
    'fused_topic', 'source_records'
  ]);
});

test('fresh combined sources fuse into one topic before Top 5 selection', () => {
  const validation = validateScheduledOutputs({
    date: '2026-07-28',
    scheduledOutputs: [
      { id: 'weather', source: 'AIOS Weather', source_time: '2026-07-28T07:00:00+08:00', summary: '大雨及雷暴' },
      { id: 'transport', source: 'AIOS Transport', source_time: '2026-07-28T07:10:00+08:00', summary: '部分道路較慢' }
    ]
  }, NOW);
  const fusion = fuseSourceTopic(validation);
  assert.equal(fusion.label, '今日安全同出行安排');
  assert.equal(fusion.source_records.length, 2);
  ...
  assert.equal(result.board.fused_topic, fusion.label);
  assert.equal(result.board.combined_source_count, 2);
  assert.equal(result.board.items.length, 5);
});
