...
test('production root loads the relationship engine without replacing the stable dashboard', async () => {
  ...
  assert.match(html, /src="relationship-app\.mjs\?v=4\.0\.0"/);
  ...
  assert.match(html, /meta name="app-version" content="4\.0\.0"/);
});
...
test('root integration reuses canonical storage and engine instead of duplicating data', async () => {
  ...
  assert.match(module, /recordMigration\(localStorage\)/);
  assert.match(module, /STATE\.generated\.unshift\(\.\.\.generated\)/);
